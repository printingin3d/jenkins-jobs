    ...
    stages {
        ...
        stage("backup HouseElf"){
            ...
            steps{
                ...
                if [ "$DOM" -eq 1 ]; then
                    # First day of the month: This is our permanent Monthly backup
                    FILENAME="monthly_$(date +%Y%m)"
                elif [ "$DOW" -eq 7 ]; then
                    # It's Sunday: This is our Weekly backup
                    FILENAME="weekly_$(date +%Y%W)"
                else
                    # Standard weekday: This is a Daily backup
                ...
                rm -f houseelf*.tgz
                tar cfz houseelf_${FILENAME}.etc.tgz --ignore-failed-read /backups/host-etc/
                echo "put houseelf_${FILENAME}.etc.tgz /public/backup/houseelf_backup/" | sftp -P 9333 -o StrictHostKeyChecking=no -i /home/jenkins/.ssh/jenkins_agent_key server@192.168.0.165
    
                rm -f houseelf*.sql.gz
                mariadb-dump houseelf -h 192.168.0.100 --skip-ssl -u houseelf -ph0us33lf --compact --result-file=houseelf_${FILENAME}.sql
                gzip --best houseelf_${FILENAME}.sql
                echo "put houseelf_${FILENAME}.sql.gz /public/backup/houseelf_backup/" | sftp -P 9333 -o StrictHostKeyChecking=no -i /home/jenkins/.ssh/jenkins_agent_key server@192.168.0.165
